...
import { fetchAccounts, fetchPrices, getUUID, initialiseAccounts, root_account } from "./account_data";
...
export async function updatePrice(commodity: commodities) {
  ...
  if (commodity.quote_flag && commodity.quote_source == "alphavantage") {
    ...
    try {
      ...
      const res = await fetch(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${commodity.mnemonic}&apikey=${api_key}`);
      if (res.ok) {
  ...
  } else if (commodity.quote_flag && commodity.quote_source == "morningstaruk") {
    console.log(`Fetching latest price for ${commodity.mnemonic}`);
    try {
      const mstar_search_regex = '<td class="msDataText searchLink"><a href="(.*?)">(.*?)</a></td><td class="msDataText searchIsin"><span>(.*)</span></td>';

      const mstar_nav_regex = '<td class="line heading">NAV<span class="heading"><br />([0-9]{2}/[0-9]{2}/[0-9]{4})</span>.*([A-Z]{3}).([0-9.]+)';

      var res = await fetch(`http://www.morningstar.co.uk/uk/funds/SecuritySearchResults.aspx?search=${commodity.mnemonic}`);
      if (res.ok) {
        ...
        if (matches) {
          ...
          if (res.ok) {
            ...
            if (matches) {
              ...
              res = await fetch(`http://www.morningstar.co.uk${next_url}&InvestmentType=SA`);
              if (res.ok) {
                console.log(`Fetched from ${next_url} OK`);
                const matches = (await res.text()).match(mstar_nav_regex);
                if (matches) {
